 ...
 * reduces path precision, and prefixes every id with `hh-<slug>-` so two inlined
 * hedgehogs never share clip-path / gradient ids on the same page.
 */
export function optimizeSvg(raw: string, slug: string): OptimizedSvg {
  ...
  const result = optimize(withViewBox, {
    ...
    plugins: [
      ...
      { name: "prefixIds", params: { prefix: `hh-${slug}`, delim: "-" } },
    ],
  });
